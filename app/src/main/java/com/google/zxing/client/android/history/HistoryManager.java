...
public final class HistoryManager {
	...
	public void addHistoryItem(Result result, ResultHandler handler) {
		// Do not save this item to the history if the preference is turned off, or the
		// contents are
		// considered secure.
	...
	public void trimHistory() {
		...
		try {
			...
			// We're seeing an error here when called in CaptureActivity.onCreate() in rare
			// cases
			// and don't understand it. First theory is that it's transient so can be safely
			// ignored.
			Log.w(TAG, sqle);
			// continue
		} finally {
	...
	/**
	 * <p>
	 * Builds a text representation of the scanning history. Each scan is encoded on
	 * one line, terminated by a line break (\r\n). The values in each line are
	 * comma-separated, and double-quoted. Double-quotes within values are escaped
	 * with a sequence of two double-quotes. The fields output are:
	 * </p>
	 *
	 * <ol>
	 * <li>Raw text</li>
	...
	CharSequence buildHistory() {
		...
		try {
			...
			while (cursor.moveToNext()) {
				...
				// Above we're preserving the old ordering of columns which had formatted data
				// in position 5
